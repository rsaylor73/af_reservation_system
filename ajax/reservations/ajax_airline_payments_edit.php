...
if ($logged == "TRUE") {
	$sql = "SELECT `airline_paymentID`,DATE_FORMAT(`payment_date`,'%Y-%m-%d') AS 'payment_date',`payment_amount`,`payment_type`,`comment` FROM `airline_payments` WHERE `airline_paymentID` = '$_GET[id]'";
	...
	while ($row = $result->fetch_assoc()) {
		...
		function update_payment(myform) {
			$.get('/ajax/reservations/ajax_airline_update_payments.php',
			$(myform).serialize(),
			function(php_msg) {
