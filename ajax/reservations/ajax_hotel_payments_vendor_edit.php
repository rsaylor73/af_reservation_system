...
if ($logged == "TRUE") {
	$sql = "SELECT `hotel_paymentID`,DATE_FORMAT(`vendor_payment_date`,'%Y-%m-%d') AS 'vendor_payment_date',`vendor_payment_amount`,`vendor_payment_type`,`vendor_comment` FROM `hotel_payments` WHERE `hotel_paymentID` = '$_GET[id]'";
	...
	while ($row = $result->fetch_assoc()) {
		...
		function update_vendor_payment(myform) {
			$.get('/ajax/reservations/ajax_hotel_update_vendor_payments.php',
			$(myform).serialize(),
			function(php_msg) {
